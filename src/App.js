...
function AppContent() {
    ...
    const handleRateMovie = async (movie, ratings, review, timeframe) => { 
        ...
        const updatePayload = {};
        if (ratings.adultRating) updatePayload[`ratings.${USERS['Kate'].name}`] = ratings.adultRating;
        if (ratings.kidRating) updatePayload[`ratings.${USERS['Ellie'].name}`] = ratings.kidRating;
