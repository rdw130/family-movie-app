...
import { getFirestore, collection, doc, setDoc, onSnapshot, serverTimestamp, writeBatch, Timestamp, updateDoc } from 'firebase/firestore';
...
function AppContent() {
    ...
    const handleRateMovie = async (movie, ratings, review, timeframe) => { 
        ...
        if (ratings.adultRating) {
            updatePayload[`ratings.${USERS[currentUser].type === 'adult' ? currentUser : 'Ryan'}`] = ratings.adultRating; // Default to Ryan if kid is rating
        }
        if (ratings.kidRating) {
             updatePayload[`ratings.${USERS[currentUser].type === 'kid' ? currentUser : 'Ellie'}`] = ratings.kidRating; // Default to Ellie if adult is rating
        }
        ...
        try { await updateDoc(docRef, updatePayload, { merge: true }); } catch (error) { console.error("Error updating rating:", error); } 
    };
