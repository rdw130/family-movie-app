...
function AppContent() {
    ...
    const handleRateMovie = async (movie, ratings, review, timeframe) => { 
        ...
        if (ratings.adultRating) {
            updatePayload[`ratings.${USERS[currentUser].type === 'adult' ? currentUser : 'Ryan'}`] = ratings.adultRating;
        }
        if (ratings.kidRating) {
             updatePayload[`ratings.${USERS[currentUser].type === 'kid' ? currentUser : 'Ellie'}`] = ratings.kidRating;
        }
